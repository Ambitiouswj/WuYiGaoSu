...
void Widget2::on_pushButton_5_clicked()
{
    this->close();
}

void Widget2::CarShow(QVector<CarRecord> &obj,QDateTime begin,QDateTime end)
{
...
void Widget2::PersonShow(QVector<PersonRecord> &obj,QDateTime begin,QDateTime end)
{
    ui->tableWidget->setRowCount(0);
    for(auto item:obj)
    {
...
void Widget2::on_data_clicked()
{
    qDebug()<<"111";
    Testchart *t12=new Testchart();
    qDebug()<<"222";
    t12->show();
...
void Widget2::on_reset_clicked()
{
    ...
    if(arg1=="车流统计")
    {
        ...
        heads<<QStringLiteral("设备编号")<<QStringLiteral("探测时间")
             <<QStringLiteral("桩号")<<QStringLiteral("车速(km/h)")
             <<QStringLiteral("车牌号");
        ui->tableWidget->setColumnCount(5);
        ui->tableWidget->setHorizontalHeaderLabels(heads);
